    ...
    if saved is None:
        with open(os.devnull, "w", encoding="utf-8") as sink:
            with redirect_stdout(sink), redirect_stderr(sink):
                ...
                protocol.prepare_test_features(device, dtype, batch_size, force_rebuild_cache, model, tokenizer, native_extract)
                protocol.prepare_training_features(seed, device, dtype, batch_size, force_rebuild_cache, model, tokenizer, native_extract)
