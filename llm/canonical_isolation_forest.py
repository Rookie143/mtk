...
class PyTorchIsolationForest(nn.Module):
    ...
    def _compile_trees(self, device):
        ...
        depth = torch.zeros((tree_count, max_nodes), dtype=torch.float32, device=device)
        leaf_extra = torch.zeros((tree_count, max_nodes), dtype=torch.float32, device=device)
        ...
        for tree_index, (tree, selected_features) in enumerate(
            ...
            depth[tree_index, :node_count] = torch.as_tensor(
                tree["node_depths"], dtype=torch.float32, device=device
            )
            leaf_extra[tree_index, :node_count] = torch.as_tensor(
    ...
    def score_samples(self, X):
        ...
        depths = (
                compiled["depth"][tree_indices, nodes]
                + compiled["leaf_extra"][tree_indices, nodes]
        ).sum(dim=1)
