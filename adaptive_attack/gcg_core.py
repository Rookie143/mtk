...
def filter_ids(ids: Tensor, tokenizer: transformers.PreTrainedTokenizer):
    ...
    if not filtered_ids:
        # This can occur with some tokenizers and a poor initialization string.
        raise RuntimeError(
...
class GCG:
    ...
    def run(
        ...
        """Run GCG and return the best optimized suffix.

        ``messages`` may be a plain user prompt or a chat-message list. If no
        ``{optim_str}`` placeholder is present, the optimized suffix is appended to
        the final message.
        """
        model = self.model
        ...
        if config.probe_sampling_config:
            assert (
                self.draft_model
                and self.draft_tokenizer
                and self.draft_embedding_layer
            ), "draft model components are not initialized"
            ...
            if config.use_prefix_cache:
                with torch.no_grad():
                    output = self.draft_model(
                        inputs_embeds=self.draft_before_embeds,
                        use_cache=True,
                    )
        ...
        for _ in tqdm(range(config.num_steps), desc="GCG optimization"):
            ...
            with torch.no_grad():
                ...
                if self.config.probe_sampling_config is None:
                    loss = find_executable_batch_size(
                        self._compute_candidates_loss_original,
                        batch_size,
                    )(input_embeds)
                    ...
                    current_loss, optim_ids = find_executable_batch_size(
                        self._compute_candidates_loss_probe_sampling,
                        batch_size,
            ...
            if self.stop_flag:
                logger.info("Early stopping: target output matched exactly")
                break
        ...
        if isinstance(config.optim_str_init, str):
            ...
            if config.buffer_size > 1:
                ...
                init_buffer_ids = torch.cat(
                    [init_optim_ids, init_buffer_ids[init_indices]],
                    dim=0,
                )
            else:
    ...
    def compute_token_gradient(
        ...
        """Compute GCG loss gradients with respect to token one-hot vectors."""
        model = self.model
        embedding_layer = self.embedding_layer
    ...
    def _compute_candidates_loss_probe_sampling(
        ...
        def _compute_draft_losses(
            ...
            ), "Draft model and embedding layer were not initialized properly."
